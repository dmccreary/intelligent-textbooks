...
This project uses `companyAnnouncements` in `.Codex/settings.local.json` to display available skills when a new conversation begins:
...
This message appears at the start of each new Codex session, helping users discover project-specific skills. The message does not appear when using the `--continue` option to resume a previous conversation.
